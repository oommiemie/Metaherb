...
import { MapPin, Truck, CreditCard, QrCode, Landmark, Banknote, Tag, MoreHorizontal, Coins, AlertCircle } from "lucide-react";
...
export function PaymentPage() {
  ...
  const coinRequested = Math.max(0, Number(coinInput) || 0);
  const insufficient = coinRequested > userCoins;
  // When the user types more than they have, ignore the input for the discount
  // calculation (don't apply a partial deduction) and surface a red error.
  const coinUsed = insufficient ? 0 : coinRequested;
  ...
  const handleConfirm = () => {
    if (insufficient) {
      toast.error("Coin ไม่เพียงพอ — กรุณาแก้ไขจำนวน Coin ที่ต้องการใช้");
      return;
    }
    if (items.length === 0) {
      toast.error(t("pay_empty"));
      return;
    }
          ...
          <div className="bg-white rounded-[16px] p-4 lg:sticky lg:top-[140px] flex flex-col gap-4">
            ...
            <div className="bg-[rgba(242,242,247,0.5)] flex flex-col gap-3 p-4 rounded-[16px] w-full">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <div className="flex items-center bg-white rounded-full pl-4 pr-1 h-[40px] flex-1 min-w-[180px] border border-gray-200 focus-within:border-[#319754] transition-colors">
                  {/* Expanding section (input + discount/error label) — capped by flex-1 + overflow so
                      it never pushes the ใช้ทั้งหมด button out. mr-2 = 8px gap before the button. */}
                  <div className="flex items-center flex-1 min-w-0 overflow-hidden mr-2">
                    <input
                      type="number"
                      inputMode="numeric"
                      min={0}
                      placeholder={`${COIN_PER_BAHT.toLocaleString()} เหรียญ = ฿1`}
                      value={coinInput}
                      onChange={(e) => setCoinInput(e.target.value)}
                      style={coinRequested > 0 ? { width: `${Math.max(1, coinInput.length)}ch` } : undefined}
                      className={`${font} ${coinRequested > 0 ? "shrink-0" : "flex-1 min-w-0"} text-[13px] outline-none bg-transparent tabular-nums ${insufficient ? "text-[#ff3b30]" : ""} [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none`}
                    />
                    {insufficient ? (
                      <span className={`${font} text-[12px] text-[#ff3b30] tabular-nums shrink-0`} style={{ fontWeight: 600, marginLeft: "2px" }}>
                        = Coin ไม่เพียงพอ
                      </span>
                    ) : coinUsed > 0 ? (
                      <span className={`${font} text-[12px] text-[#319754] tabular-nums shrink-0`} style={{ fontWeight: 600, marginLeft: "2px" }}>
                        = ส่วนลด ฿{coinDiscount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </span>
                    ) : null}
                  </div>
                  <button type="button" onClick={() => setCoinInput(String(userCoins))}
                    className={`${font} bg-[#f7931d]/10 text-[#f7931d] text-[11px] px-3 h-7 rounded-full cursor-pointer hover:bg-[#f7931d]/20 transition-colors shrink-0`}
            ...
            {coinUsed > 0 && (
              <div className="flex justify-between px-4">
                ...
                <span className={`${font} text-[14px] text-[#34c759] tabular-nums`} style={{ fontWeight: 500 }}>-฿{coinDiscount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
              </div>
            )}
            ...
            <button onClick={handleConfirm} disabled={insufficient}
              className={`w-full h-[48px] rounded-full text-white text-[14px] ${font} transition-colors ${
                insufficient
                  ? "bg-gray-300 cursor-not-allowed"
                  : "bg-[#319754] hover:bg-[#267a43] cursor-pointer"
              }`}>{t("pay_confirm")}</button>
          </div>
        </div>
      </div>
